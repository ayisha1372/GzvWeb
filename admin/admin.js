...
function renderMembers() {
  ...
  if (!rows.length) {
    body.innerHTML = `<tr class="empty-row"><td colspan="7">No members yet. Click "+ Add Member" to create one.</td></tr>`;
    return;
  }
...
const FORM_CONFIG = {
  member: {
    ...
    fields: [
      ...
      { key: 'subgroup', label: 'Club (optional — for a club under a Wing, e.g. "Debate Club". Leave blank for general wing members)', type: 'text' },
...
function openForm(type, id) {
  ...
  document.getElementById('modalTitle').textContent =
    (existing ? 'Edit ' : 'Add ') + config.title;
    ...
    .map((f) => {
      ...
      if (f.type === 'textarea') {
        return `
          <div class="form-group">
            <label>${f.label}</label>
            <textarea name="${f.key}">${esc(value)}</textarea>
          </div>
        `;
      }
          ...
          <div class="form-group">
            ...
            <div class="image-upload-box">
              ...
              <div class="image-upload-row">
                <label
                  class="btn-outline image-upload-btn"
                  for="imageFile-${f.key}">
                  Choose Image
                </label>
...
function setupImageUploadFields() {
  document.querySelectorAll('.image-file-input').forEach((input) => {
    const key = input.name.replace(/_file$/, '');

    const urlInput = document.querySelector(
      `.image-url-input[name="${key}"]`
    );

    const previewWrap = document.querySelector(
      `[data-image-preview-wrap="${key}"]`
    );
...
document.getElementById('entityForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    for (const f of config.fields) {
      ...
      const file = formData.get(`${f.key}_file`);

      if (file instanceof File && file.size > 0) {
        saveBtn.textContent = 'Uploading…';

        payload[f.key] = await uploadImage(file);
      }
    }
...
document.addEventListener('click', async (e) => {
  ...
  // Find Edit button even if the user clicks an element inside it
  const editBtn = e.target.closest('[data-edit]');

  if (editBtn) {
    const editType = editBtn.dataset.edit;
    const editId = editBtn.dataset.id;

    if (!editType || !editId) {
      console.error('Edit button is missing data-edit or data-id');
      return;
    }
  ...
  if (deleteBtn) {
    ...
    const deleteType = deleteBtn.dataset.delete;
    const deleteId = deleteBtn.dataset.id;

    if (!deleteType || !deleteId) {
      console.error('Delete button is missing data-delete or data-id');
      return;
    }
